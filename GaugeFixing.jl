...
function fix_continuous_gauge(A)
	...
	SH, H = environment.eig(1, 0, hermitian = true) #wierdly, I  
...
function parity(vec::Vector, Z)
	return dot(vec, Z * vec) / (norm(vec)^2) |> round
end

function parity(δA, ZH, ZV)
	return +1
end

function parity(δA::Array{T, 4}, ZH, ZV) where {T}
	Z2δA = ncon([δA, ZH, ZV, ZH, ZV], [[1, 2, 3, 4], [1, -1], [2, -2], [3, -3], [4, -4]])
	return dot(Z2δA, δA) / (norm(δA)^2) #|> round
end

function project_Z2_even(δA::Array{T, 4}, ZH, ZV) where {T}
	Z2δA = ncon([δA, ZH, ZV, ZH, ZV], [[1, 2, 3, 4], [1, -1], [2, -2], [3, -3], [4, -4]])
	return (δA + Z2δA) / 2
end

function project_Z2_odd(δA::Array{T, 4}, ZH, ZV) where {T}
	Z2δA = ncon([δA, ZH, ZV, ZH, ZV], [[1, 2, 3, 4], [1, -1], [2, -2], [3, -3], [4, -4]])
	return (δA - Z2δA) / 2
end

#=
function sort_decomposition_by_sectors(dec::Eigen, Z)
	par = x -> parity(x, Z)
	evens = (mapslices(par, dec.vectors; dims = 1).==1)[:]
	odds = (mapslices(par, dec.vectors; dims = 1).==-1)[:]
	vecs_even = dec.vectors[:, evens]
	vecs_odd = dec.vectors[:, odds]
	values_even = dec.values[evens]
	values_odd = dec.values[odds]
	return Eigen(vcat(values_even, values_odd), hcat(vecs_even, vecs_odd))
end

function fix_continuous_gauge_Z2(A::Array, ZH, ZV)
	tensors = [A, A]

	environment = Hermitian(environment_for_vertical_gauge(tensors))
	decomposition = sort_decomposition_by_sectors(eigen(environment; sortby = x -> -abs(x)), ZV)
	SV, V = decomposition.values, decomposition.vectors
	correct_phase_of_each_column(V)
	A = ncon([A, V, V], [[-1, 2, -3, 4], [2, -2], [4, -4]])

	environment = Hermitian(environment_for_horizontal_gauge(tensors))
	decomposition = sort_decomposition_by_sectors(eigen(environment; sortby = x -> -abs(x)), ZH)
	SH, H = decomposition.values, decomposition.vectors
	correct_phase_of_each_column(H)
	A = ncon([A, H, H], [[1, -2, 3, -4], [1, -1], [3, -3]])
	return A, H, V, SH, SV
