# MODULE: critical_temperature.jl
# USAGE: julia +1.10.4 --project=ekrgilttrnr ekrgilttrnr/scripts/critical_temperature.jl [options]
# INPUTS: None (parameters from command line)
# OUTPUTS: ekrgilttrnr/critical_temperatures/*.data (serialized critical temperature)
# DESCRIPTION: Binary search to find critical temperature for 2D Ising model
...
# NEW IN THIS SCRIPT: Adapted for our directory structure with config-driven runs.
#                     Uses GILT-TNR flow to distinguish ordered/disordered phases
#                     and locate critical point via binary search.
#
# RUNTIME: ~20-30 min at χ=30 (depends on precision). Binary search convergence.
