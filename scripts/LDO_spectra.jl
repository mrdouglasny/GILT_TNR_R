# MODULE: LDO_spectra.jl
# USAGE: julia +1.10.4 --project=ekrgilttrnr ekrgilttrnr/scripts/LDO_spectra.jl [options]
# INPUTS: ekrgilttrnr/newton/*.data (serialized fixed point tensors)
# OUTPUTS: stdout (LDO eigenvalue spectrum)
# DESCRIPTION: Compute Lattice Dilatation Operator (LDO) spectrum at fixed point.
...
# NEW IN THIS SCRIPT: Adapted for our directory structure with ProjectUtils integration.
#
# RUNTIME: ~10 min at χ=30. Dominated by eigensolver for large LDO matrix.
#
# sections
# - EXPERIMENT INIT
...
if rotate == false
    function LDO(O)
        # replacing @tensor order=(.....) with @tensoropt one can get a much faster algorithm.  
