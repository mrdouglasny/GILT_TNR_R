...
for steps in 1:10
    # RG Flow
    println("Running step $steps with mu_sq = $mu_sq")
    ...
    println("Norm of A_crit_JU: $(norm(A_crit_JU))")
    println("First element of A_crit_JU: $(A_crit_JU.sects[(0,0,0,0)][1])")
    println("Sectors: $(keys(A_crit_JU.sects))")
    
    # Define projected map for linearization
    function gilt_phi4_projected(A_in)
        A_out = gilt_phi4(A_in)
        return project_to_structure(A_out, A_crit_JU)
    end

    # Linearized RG
    function linearized_RG(dA)
        out = NumDifferentiation.df(gilt_phi4_projected, A_crit_JU, dA; stp=stp, order=ord)
        return out # Already projected
    end
